...
std::expected<void, std::string> Administrator::addUser(std::vector<Project*>& projects, std::vector<User*>& users, const std::string& username, const std::string& projectName)
{
    User* user = nullptr;
    for (User* u : users) {
        if (u->getUsername() == username) { user = u; break; }
    }
    if (user == nullptr)
    ...
    for (Project* p : projects) {
        if (p->getName() == projectName) { project = p; break; }
    }
    if (project == nullptr)
...
std::expected<void, std::string> Administrator::removeUser(std::vector<Project*>& projects, std::vector<User*>& users, const std::string& username)
{
    User* userToDelete = nullptr;
    for (User* user : users) {
        if (user->getUsername() == username) { userToDelete = user; break; }
    }
